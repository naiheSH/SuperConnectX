...
export async function startMcpStdio(facade: McpFacade, policy: McpPermissionPolicy = DEFAULT_MCP_PERMISSION_POLICY): Promise<() => Promise<void>> {
  const server = createMcpServer(facade, policy); await server.connect(new StdioServerTransport()); return () => server.close()
}
...
export class McpLoopbackServer {
  ...
  constructor(private readonly facade: McpFacade, token = crypto.randomBytes(32).toString('hex'), private readonly policy: McpPermissionPolicy = DEFAULT_MCP_PERMISSION_POLICY) { this.token = Buffer.from(token) }
  ...
  private async handle(req: IncomingMessage, res: ServerResponse) {
    try {
      ...
      if (req.method === 'POST' && !sessionId) { const body = await this.body(req); const transport = new StreamableHTTPServerTransport({ sessionIdGenerator: () => crypto.randomUUID() }); const server = createMcpServer(this.facade, this.policy); const close = async () => server.close(); await server.connect(transport); await transport.handleRequest(req, res, body); if (transport.sessionId) this.sessions.set(transport.sessionId, { transport, close }); return }
