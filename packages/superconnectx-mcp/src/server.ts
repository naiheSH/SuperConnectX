...
export function createMcpServer(facade: McpFacade, policy: McpPermissionPolicy = DEFAULT_MCP_PERMISSION_POLICY): McpServer {
  ...
  server.registerTool('log_summarize', { title: 'Summarize log', description: 'Summarize bounded log evidence.', inputSchema: { sessionId: z.string().min(1), maxBytes: z.number().int().min(1).max(1048576).optional() }, annotations: { readOnlyHint: true, idempotentHint: true } }, async ({ sessionId, maxBytes }) => result(await facade.summarizeLog?.(sessionId, { maxBytes })))
  server.registerTool('log_find_anomalies', { title: 'Find log anomalies', description: 'Find common error, timeout, restart and disconnect evidence.', inputSchema: { sessionId: z.string().min(1), maxBytes: z.number().int().min(1).max(1048576).optional() }, annotations: { readOnlyHint: true, idempotentHint: true } }, async ({ sessionId, maxBytes }) => result(await facade.findLogAnomalies?.(sessionId, { maxBytes })))
  server.registerTool('log_compare', { title: 'Compare logs', description: 'Compare bounded evidence from two sessions.', inputSchema: { leftSessionId: z.string().min(1), rightSessionId: z.string().min(1), maxBytes: z.number().int().min(1).max(1048576).optional() }, annotations: { readOnlyHint: true, idempotentHint: true } }, async ({ leftSessionId, rightSessionId, maxBytes }) => result(await facade.compareLogs?.(leftSessionId, rightSessionId, { maxBytes })))
  server.registerTool('session_start_port', { title: 'Start serial session', description: 'Start an AI-owned serial session.', inputSchema: { port: z.string().min(1).max(255), baudRate: z.number().int().min(1).max(4000000).optional(), ownerId: z.string().min(1) } }, async ({ port, baudRate, ownerId }) => result(await facade.startSessionPort?.(port, baudRate, ownerId)))
  server.registerTool('session_start_saved', { title: 'Start saved session', description: 'Start an AI-owned saved connection.', inputSchema: { connectionId: z.number().int().positive(), ownerId: z.string().min(1) } }, async ({ connectionId, ownerId }) => result(await facade.startSavedSession?.(connectionId, ownerId)))
  ...
  server.registerTool(
    ...
    async ({ sessionId, command, ownerId, wait }) => {
      ...
      return result(await facade.sendSessionAndWait?.(sessionId, command, wait))
    }
  )
  server.registerTool('session_run_template_command', { title: 'Run template command', description: 'Run a declarative template command with a write lease.', inputSchema: { sessionId: z.string().min(1), templateId: z.string().min(1), commandId: z.string().min(1), ownerId: z.string().min(1) } }, async ({ sessionId, templateId, commandId, ownerId }) => { lease(facade, sessionId, ownerId); return result(await facade.runTemplateCommand?.(sessionId, templateId, commandId, ownerId)) })
